...
import 'package:furtive_pygmy/const.dart';
import 'package:furtive_pygmy/widget/full_photo.dart';
import 'package:furtive_pygmy/widget/loading.dart';
...
class ChatScreenState extends State<ChatScreen> {
  ...
  void onSendMessage(String content, int type) {
    ...
    if(type == 0) {
      ...
      for(int i = 0; i< content.length; i++) {
        int temp = content.codeUnitAt(i);
        int offset;
        String h;
        if(temp >= 'a'.codeUnits[0] && temp <= 'z'.codeUnitAt(0)) {
          offset = 97;
        } else if(temp >= 'A'.codeUnits[0] && temp <= 'Z'.codeUnitAt(0)) {
          offset = 65;
        } else if (temp == ' '.codeUnitAt(0)) {
          temp2 += " ";
          continue;
        } else {
          temp2 += content[i];
        ...
        int cipher;
        cipher = (temp + caesarKey - offset) % 26;
        h = String.fromCharCode(cipher + offset);
        temp2 += h;
      }
      content = temp2;
    }
                ...
                document.data()['type'] == 0
                  ...
                  child: InkWell(
                    ...
                    onTap: (){
                      ...
                      print(decryptMessage(document.data()['content']));
                      Fluttertoast.showToast(msg: decryptMessage(document.data()['content']), backgroundColor: Colors.black, textColor: Colors.red);
                    },
                  ),
                  padding: EdgeInsets.fromLTRB(15.0, 10.0, 15.0, 10.0),
                  width: 200.0,
  ...
  Widget buildInput() {
    return Container(
        ...
        children: <Widget>[
          ...
          Flexible(
            child: Container(
              ...
              decoration: BoxDecoration(color: textboxColor, borderRadius: BorderRadius.circular(8.0)),
            ),
          ),
      ...
      height: 50.0,
      decoration: BoxDecoration(border: Border(top: BorderSide(color: greyColor2, width: 0.5)), color: themeColor),
    );
  }
